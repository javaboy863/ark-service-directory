...
import com.ark.service.apidocs.annotations.ApiModule;
import com.ark.service.apidocs.annotations.RequestParam;
import com.ark.service.apidocs.core.beans.ApiCacheItem;
import com.ark.service.apidocs.core.beans.ApiParamsCacheItem;
import com.ark.service.apidocs.core.beans.HtmlTypeEnum;
import com.ark.service.apidocs.core.beans.ModuleCacheItem;
import com.ark.service.apidocs.core.beans.ParamBean;
import com.ark.service.apidocs.core.providers.DubboDocProviderImpl;
import com.ark.service.apidocs.core.providers.IDubboDocProvider;
...
public class DubboApiDocsAnnotationScanner implements ApplicationListener<ApplicationReadyEvent> {
        ...
        apiModules.forEach((key, apiModuleTemp) -> {
            ...
            if (apiModuleClass.isAnnotationPresent(Service.class)) {
                ...
                for (ServiceConfig serviceConfig : serviceConfigs) {
                    ...
                    if(moduleAnn.apiInterface() ==  serviceConfig.getInterfaceClass()){
                        async = serviceConfig.isAsync()==null?false:serviceConfig.isAsync();
                        apiVersion = serviceConfig.getVersion();
                        apiGroup = serviceConfig.getGroup();
                        break;
                    }
                }
    ...
    private void processApiDocAnnotation(Method method, List<ApiCacheItem> moduleApiList, ApiModule moduleAnn,
        ...
        for (int i = 0; i < argsClass.length; i++) {
            ...
            methodParamInfoSb.append(Constants.METHOD_PARAM_INDEX_BOUNDARY_LEFT).append(i)
                .append(Constants.METHOD_PARAM_INDEX_BOUNDARY_RIGHT).append(argClass.getCanonicalName());
            if (i + 1 < argsClass.length) {
                methodParamInfoSb.append(Constants.METHOD_PARAMETER_SEPARATOR);
            }
    ...
    private String getMethodParamsDesc(Method method) {
        ...
        if (Constants.NO_ARGS_NO_RETURN_VALUES_METHOD_DESC.equals(desc)) {
            desc = "";
        }
        return desc;
    }
    ...
    private ParamBean processHtmlType(Class<?> classType, RequestParam annotation, ParamBean param) {
        ...
        if (Boolean.class.isAssignableFrom(classType) || boolean.class.isAssignableFrom(classType)) {
            ...
            param.setAllowableValues(new String[]{Constants.ALLOWABLE_BOOLEAN_TRUE, Constants.ALLOWABLE_BOOLEAN_FALSE});
            processed = true;
